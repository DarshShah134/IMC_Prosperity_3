...
class Trader:
    ...
    def get_orders_jams(self, state: TradingState, trader_state: dict) -> List[Order]:
        ...
        # fair value from params (fallback to current mid)
        fv = self.params.get(product, {}).get("fair_value", mid_price if mid_price is not None else 0)

        # ── trading rules (identical to second algo) ──────────────────────────
        # Buy if the best ask is below FV
        if best_ask is not None and best_ask < fv:
            qty = min(limit - position, -od.sell_orders[best_ask], DEFAULT_TRADE_SIZE)
            if qty > 0:
                orders.append(Order(product, best_ask, qty))

        # Sell if the best bid is above FV
...
class IgneousStoneCouponManager:
    """
    Volcanic‑Rock / Voucher market‑maker
    (unchanged except we now respect position limits)
    """

    # hard limits you asked for
    POSITION_LIMITS = {
        "VOLCANIC_ROCK": 400,
    ...
    def run(
        self, snapshot: TradingState
    ) -> Tuple[Dict[str, List[Order]], int, str]:

        out_orders: Dict[str, List[Order]] = {}
        memo_state = ""          # we don’t keep state yet
        total_convs = 0          # no conversions needed here

        # ────────────────── reference price for rock ────────────────────
        base_rock_quote = None
        ...
        if rock_depth:
            if rock_depth.buy_orders and rock_depth.sell_orders:
                base_rock_quote = (max(rock_depth.buy_orders)
                                   + min(rock_depth.sell_orders)) / 2.0
        ...
        for cpn, strike in self.COUPON_STRIKES.items():
            ...
            if depth.sell_orders:
                ...
                if best_ask < intrinsic and pos_now < limit:
                    avail = min(-depth.sell_orders[best_ask],  # liquidity
                                limit - pos_now)               # room to buy
                    if avail > 0:
            ...
            if depth.buy_orders:
                ...
                if best_bid > intrinsic and pos_now > -limit:
                    avail = min(depth.buy_orders[best_bid],    # liquidity
                                limit + pos_now)               # room to sell
                    if avail > 0:
