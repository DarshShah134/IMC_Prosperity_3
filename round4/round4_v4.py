...
class Trader:
    def __init__(self) -> None:
        ...
        # Generic strategies (HamperDealApproach) — now only JAMS & PICNIC_BASKET2
        self.strategies: Dict[Symbol, ApproachBase] = {
            sym: cls(sym, caps[sym])
            for sym, cls in {
                # "JAMS": HamperDealApproach,
                "PICNIC_BASKET2": HamperDealApproach,
                # "ORCHIDS": FlowerArrangementMethod,  # still commented
            }.items()
        }

        # Custom single‑symbol logic
        ...
        # Spread arbitrage manager (handles PICNIC_BASKET1, CROISSANTS, DJEMBES)
    ...
    def run(self, snap: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        # Restore basket state if present
        if "BASKET_SPREAD" in old_persist:
            try:
                self.basket_manager.state = jsonpickle.decode(old_persist["BASKET_SPREAD"])
            except Exception:
                self.basket_manager.state = {}
        else:
            self.basket_manager.state = {}
        ...
        for sym, strat in self.strategies.items():
            if sym in old_persist:
                strat.restore(old_persist[sym])
            if sym in snap.order_depths:
                orders, convs = strat.process(snap)
                if orders:
                    final_request[sym] = orders
                conv_sum += convs
            new_persist[sym] = strat.preserve()

        # -------------------------------------- single symbol special logic
        for sym, strat in self.special_strategies.items():
            if sym in snap.order_depths:
                orders, _ = strat.process(snap)
                if orders:
                    final_request[sym] = orders

        # -------------------------------------- basket spread manager
        basket_orders, _, basket_data = self.basket_manager.run(snap)
        for sym, ords in basket_orders.items():
            final_request.setdefault(sym, []).extend(ords)
        new_persist["BASKET_SPREAD"] = basket_data

        # -------------------------------------- coupon manager
        coupon_orders, coupon_convs, _ = self.coupon_machine.run(snap)
        for sym, ords in coupon_orders.items():
            final_request.setdefault(sym, []).extend(ords)
